...
const Blog = () => {
    ...
    useEffect(() => {
        ...
        client.fetch(`*[_type == "blogPost"] | order(date desc)[0...3]`).then((data) => {
            setSanityPosts(data);
        }).catch(console.error);
    }, []);
            ...
            <div key={post._id}>
                <Link to={`/blog/${post._id}`} className="group flex flex-col sm:flex-row gap-6 sm:gap-8 items-start sm:items-center">
                    <div className="w-full sm:w-[280px] shrink-0 overflow-hidden rounded-lg bg-white dark:bg-slate-900 ring-1 ring-neutral-200 dark:ring-neutral-800 shadow-sm">
                        {imageUrl ? (
                            <img src={imageUrl} alt={title} className="w-full h-48 sm:h-36 object-cover transition-transform duration-700 group-hover:scale-105 opacity-90 group-hover:opacity-100" />
                        ) : (
    ...
    return (
        <section id="blog" className="py-32 px-4 md:px-8 max-w-7xl mx-auto bg-background text-foreground relative">
            <motion.div
            ...
            >
                ...
                <div className="flex flex-col gap-8 md:gap-12">
                    {sanityPosts.length > 0 ? (
                        ...
                        <p className="text-neutral-500 italic">No posts found. Please add posts to Sanity CMS.</p>
                    )}
                </div>
            </motion.div>
        </section>
